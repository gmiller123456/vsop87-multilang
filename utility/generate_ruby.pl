...
sub produceFile {
    ...
    open($f,">$filename.rb");
    open($h,">$filename"."_velocities.rb");
...
sub generateCoordFunctions{
    ...
    while($l=<$in>){
        ...
        if($var ne $lastVar){
            print $f "   def self.$planet"."_$var(t)\r\n";
            print $h "   def self.$planet"."_$var(t)\r\n";
        }
        ...
        if($vars[4]>= $precision){
            ...
            if($exponent==0){
                ...
                print $h "      $currentVar += (t**".($exponent-1).") * $exponent * $vars[4] * Math.cos($vars[5] + $vars[6]*t) -";
                print $h " (t**$exponent) * $vars[4] * $vars[6] * Math.sin($vars[5] + $vars[6]*t)\r\n";
            }
            $exponentHasData=1;
        }
...
sub fileStart{
    ...
    print $f "class ".ucfirst($currentVersion)."_".$precisionName."\r\n";
    print $h "class ".ucfirst($currentVersion)."_".$precisionName."_velocities\r\n";
 }
