...
class Generator:
    ...
    def writeVariables(self,planet,variablesList):
        ...
        for variable in variables:
            self.bprintln(f"    @JvmStatic private fun {planet}_{variable}(t: Double): Double {{")
    ...
    def writeGetters(self,data):
        for planet in list(data):
            ...
            for i,variable in enumerate(list(data[planet])):
                self.position.write(f"        temp[{i}]={planet}_{variable}(t);\r\n")
                if(self.velocity!=None):
                    self.velocity.write(f"        temp[{i}]={planet}_{variable}(t) / 365250.0;\r\n")
