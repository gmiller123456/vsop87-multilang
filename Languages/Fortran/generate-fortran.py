...
class Generator:
    ...
    def getPositionLine(self,variable,a,b,c):
        return f"        {variable} = {variable} + {a: > 18}*cos({b: > 14} + {c: > 20} * t)\r\n"

    def getVelocityLine(self,variable,a,b,c,exponent):
        t=f"-pow(t,{exponent})*{a: > 18}*{c: > 20}*sin({b: > 14} + {c: > 20} * t)"
        if exponent==0:
            return f"        {variable} = {variable} + {t}\r\n"
        else:
            return f"        {variable} = {variable} + pow(t,{exponent-1})*{exponent}*{a: > 18}*cos({b: > 14} + {c: > 20}*t){t}\r\n"
    ...
    def writeGetters(self,data):
        for planet in list(data):
            self.bprintln(f"    subroutine get{planet.capitalize()}(t, result)")
            self.bprintln("        double precision :: t")
            self.bprintln("        double precision, dimension(3) :: result\r\n")
