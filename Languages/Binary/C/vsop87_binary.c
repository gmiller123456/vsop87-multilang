...
/*
Only one version of VSOP can be loaded at any given time!

The caller, must call vsop87_binary_init(filename); before making any other calls.

The caller must call vsop87_binary_free() when done, or before loading a different version.
*/
...
int guessVersion(char* filename){
    if (strstr(filename,"vsop87a") != NULL) return 1;
    if (strstr(filename,"vsop87b") != NULL) return 2;
    if (strstr(filename,"vsop87c") != NULL) return 3;
    if (strstr(filename,"vsop87d") != NULL) return 4;
    if (strstr(filename,"vsop87e") != NULL) return 5;
    if (strstr(filename,"vsop87") != NULL) return 0;
    return 0;
}

//Returns 0 on success
int loadVSOPData(char* filename){
...
void getMoon(double earth[], double emb[], double out[]){
    ...
    out[3] = (emb[3] - earth[3]) * (1 + 1 / 0.01230073677);
    out[4] = (emb[4] - earth[4]) * (1 + 1 / 0.01230073677);
    out[5] = (emb[5] - earth[5]) * (1 + 1 / 0.01230073677);
    out[3] = out[3] + earth[3];
    out[4] = out[4] + earth[4];
    out[5] = out[5] + earth[5];
}
...
void getPlanet(int* offsets, double t,double* out){
    double* p = out;
    for (int i = 0; i < varcount; i++){
        ...
        for (int j = 0; j < 6; j++)
        {
            ...
            for (int k = 0; k < length; k++)
            {
                ...
                {
                    veacc += pow(t, j - 1) * j * a * cos(b + c * t) - pow(t, j) * a * c * sin(b + c * t);
                }
                o += 3;
            }
            acc += eacc * pow(t, j);
        ...
        p[i] = (acc);
        if(varcount==3) p[i + varcount] = (veacc / 365250.0);
    }
}

void assignOffsets(int version){
...
void vsop87_binary_getEarth(double t,double out[]){
    return getPlanet(offsets_earth, t, out);
}

void vsop87_binary_getEmb(double t,double out[]){
    return getPlanet(offsets_emb, t, out);
}

void vsop87_binary_getJupiter(double t,double out[]){
    return getPlanet(offsets_jupiter, t, out);
}

void vsop87_binary_getMars(double t,double out[]){
    return getPlanet(offsets_mars, t, out);
}

void vsop87_binary_getMercury(double t,double out[]){
    return getPlanet(offsets_mercury, t, out);
}

void vsop87_binary_getNeptune(double t,double out[]){
    return getPlanet(offsets_neptune, t, out);
}

void vsop87_binary_getSaturn(double t,double out[]){
    return getPlanet(offsets_saturn, t, out);
}

void vsop87_binary_getSun(double t,double out[]){
    return getPlanet(offsets_sun, t, out);
}

void vsop87_binary_getUranus(double t,double out[]){
    return getPlanet(offsets_uranus, t, out);
}

void vsop87_binary_getVenus(double t,double out[]){
    return getPlanet(offsets_venus, t, out);
}

int vsop87_binary_init(char* _filename){
    version = guessVersion(_filename);
    assignOffsets(version);
    int status=loadVSOPData(_filename);
    return 0;
}
